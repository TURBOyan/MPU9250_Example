...
const byte F8X16[]=
{
	...
	0x00,0x00,0x80,0x80,0x80,0x80,0x80,0x00,0x00,0x33,0x24,0x24,0x24,0x24,0x19,0x00,//char3
...
void OLED_P6x8Int(uint8_t x, uint8_t y, int16_t data1, int8 set)
{
  ...
  if(set < 0)
  {
    ...
    LED_P6x8Str(x, y, (char *)&(result[5 + set]));
  }
  else if(set > 0)
  {
    LED_P6x8Str(x, y, (char *)&(result[6 - set]));
  }
}
...
void OLED_P6x8Flo(uint8_t x, uint8_t y, float data1, int8 set)
{
    ...
    result[8] = '\0';
		...
		if(set < 0)
		{
			...
			LED_P6x8Str(x, y, (char *)&(result[4 + set]));
		}
		else if(set > 0)
		{
			LED_P6x8Str(x, y, (char *)&(result[5 - set]));
...
void LED_PrintValueI(uint8 x, uint8 y, int data)
{
 ...
 void LED_PrintValueFP(uint8 x, uint8 y, uint data, uint8 num)
 {
 	uint8 m,i,j,k;  	
 	LED_P6x8Char(x, y, '.');
	m= data/1000;
...
//==============================================================
 void LED_PrintValueF(uint8 x, uint8 y, float data, uint8 num)
 {
 	...
 	//整数部分显示
 	if(data < 0.0000001f)  LED_P6x8Char(x, y,'-'); 
...
if((pucTable+(((usRowTmp + k)*usRowNum)/64)*usColumnNum)[(j*usColumnNum)/128]==1)   
    {
                    ucData = ucData|0x80;
                }
