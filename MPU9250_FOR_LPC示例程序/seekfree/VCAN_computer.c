...
#define CMD_WARE     3
    ...
    uart_putbuff(Bluetooth_UART, cmdf, sizeof(cmdf));    //先发送前命令
    uart_putbuff(Bluetooth_UART, (uint8_t *)wareaddr, waresize);    //发送数据
    uart_putbuff(Bluetooth_UART, cmdr, sizeof(cmdr));    //发送后命令
